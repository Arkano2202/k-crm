<?php
// actualizar_leads.php (en la raíz)
$pagina_actual = 'subir_leads'; // Para que el sidebar se mantenga activo
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['archivo_actualizar'])) {
    try {
        ...
        if (!in_array($extension, ['csv'])) {
            throw new Exception('Por ahora solo se aceptan archivos CSV.');
        }
        
        // Procesar archivo CSV con punto y coma como delimitador
        $rows = [];
        ...
        // Verificar que TP esté en los encabezados
        if (!in_array('TP', $encabezados)) {
            throw new Exception('El archivo debe contener la columna "TP" para identificar los leads.');
        }
        
        // Campos permitidos para actualizar
        $campos_permitidos = ['Nombre', 'Apellido', 'Correo', 'Numero', 'Pais', 'Campaña', 'Asignado', 'Estado', 'UltimaGestion', 'FechaUltimaGestion'];
        
        // Filtrar campos que se pueden actualizar
        $campos_a_actualizar = array_intersect($encabezados, $campos_permitidos);
        
        if (count($campos_a_actualizar) === 0) {
            throw new Exception('No se encontraron campos válidos para actualizar. Campos permitidos: ' . implode(', ', $campos_permitidos));
        }
        
        // Preparar consultas dinámicamente
        $registros_procesados = 0;
        $registros_actualizados = 0;
        
        // Procesar cada fila (empezando desde la fila 1, saltando encabezados)
        foreach (array_slice($rows, 1) as $fila_index => $fila) {
            ...
            // Verificar si el TP existe y obtener datos actuales
            $check_query = "SELECT * FROM clientes WHERE TP = :tp";
            $check_stmt = $db->prepare($check_query);
            $check_stmt->bindParam(':tp', $tp);
            $check_stmt->execute();
            ...
            if ($check_stmt->rowCount() === 0) {
                $registros_no_encontrados[] = [
                    'fila' => $fila_index + 2,
                    'tp' => $tp,
                    'razon' => 'TP no encontrado en la base de datos'
                ];
                continue;
            }
            
            $cliente_actual = $check_stmt->fetch(PDO::FETCH_ASSOC);
            $nombre_cliente = $cliente_actual['Nombre'] . ' ' . $cliente_actual['Apellido'];
            $asignado_actual = $cliente_actual['Asignado'];
            ...
            foreach ($campos_a_actualizar as $campo) {
                if (!empty($datos[$campo]) && $campo !== 'TP') {
                    $update_fields[] = "$campo = :$campo";
                    $update_params[":$campo"] = $datos[$campo];
                    
                    // Si es FechaUltimaGestion, usar fecha actual
                    if ($campo === 'FechaUltimaGestion') {
                        $update_params[":$campo"] = date('Y-m-d H:i:s');
                    }
                }
            }
            ...
            if (empty($update_fields)) {
                $registros_no_encontrados[] = [
                    'fila' => $fila_index + 2,
                    'tp' => $tp,
                    'razon' => 'No hay campos válidos para actualizar'
                ];
                continue;
            }
            
            $update_query = "UPDATE clientes SET " . implode(', ', $update_fields) . " WHERE TP = :tp";
            ...
            if ($update_stmt->execute()) {
                if ($update_stmt->rowCount() > 0) {
                    ...
                    // REGISTRAR EN HISTÓRICO usando tu función
                    $accion = 'ACTUALIZACIÓN MASIVA';
                    $modulo = 'ACTUALIZAR LEADS';
                    
                    // Si se está actualizando el campo Asignado, usar el nuevo valor
                    $asignado_para_historico = isset($datos['Asignado']) && !empty($datos['Asignado']) ? $datos['Asignado'] : $asignado_actual;
                    
                    registrarHistorico($tp, $nombre_cliente, $asignado_para_historico, $accion, $modulo);
                    
                } else {
                    $registros_no_encontrados[] = [
                        'fila' => $fila_index + 2,
                        'tp' => $tp,
                        'razon' => 'No se realizaron cambios (los datos ya estaban actualizados)'
                    ];
                }
            } else {
                $registros_no_encontrados[] = [
                    'fila' => $fila_index + 2,
                    'tp' => $tp,
                    'razon' => 'Error al actualizar en base de datos'
                ];
            }
        }
        
        // Generar archivo de resultados si hay registros no procesados
        if (!empty($registros_no_encontrados)) {
            $resultados_file = generar_archivo_resultados($registros_no_encontrados);
        }
        
        $mensaje = "Proceso de actualización completado: " . 
                   "Procesados: $registros_procesados, " . 
                   "Actualizados: $registros_actualizados, " . 
                   "No procesados: " . count($registros_no_encontrados);
...
// Función para generar archivo de resultados de actualización
function generar_archivo_resultados($resultados) {
    $filename = 'resultados_actualizacion_' . date('Y-m-d_H-i-s') . '.csv';
    ...
    $fp = fopen($filepath, 'w');
    // Usar punto y coma como delimitador también para el archivo de resultados
    fputcsv($fp, ['Fila', 'TP', 'Razón'], ";");
