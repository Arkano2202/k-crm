...
if ($registros_por_pagina < 1) $registros_por_pagina = 1;
...
if (!empty($terminos_busqueda)) {
    ...
    // Búsqueda para auxiliar
    if ($search_field === 'all' || $search_field === 'auxiliar') {
        if ($es_busqueda_multiple) {
            // Búsqueda múltiple para auxiliar
            $placeholders = [];
            foreach ($terminos_busqueda as $index => $termino) {
                $param_name = ":search_auxiliar_$index";
                $placeholders[] = "c.Auxiliar LIKE $param_name";
                $params[$param_name] = "%$termino%";
            }
            $search_conditions[] = "(" . implode(" OR ", $placeholders) . ")";
        } else {
            // Búsqueda individual para auxiliar
            $search_conditions[] = "c.Auxiliar LIKE :search_auxiliar";
            $params[':search_auxiliar'] = "%{$terminos_busqueda[0]}%";
        }
    }
        ...
        <div class="search-bar">
            ...
            <form method="GET" action="leads.php" style="display: flex; align-items: center; width: 100%; gap: 10px;">
                <select name="search_field" style="border: none; background: transparent; outline: none; font-size: 14px; min-width: 150px;">
                    ...
                    <option value="auxiliar" <?php echo ($search_field === 'auxiliar') ? 'selected' : ''; ?>>Solo Auxiliar</option>
                </select>
        ...
        <div class="dashboard-card">
            <div class="card-header">
                <div class="card-title">
                    ...
                    <!-- Selector de registros por página - INPUT MANUAL -->
                    <div style="display: inline-block; margin-left: 20px;">
                        <form method="GET" action="leads.php" style="display: inline-flex; align-items: center; gap: 8px;" id="formRegistrosPorPagina">
                            <label for="registros_por_pagina" style="font-size: 12px; color: #7f8c8d; font-weight: normal;">
                                Mostrar:
                            </label>
                            <input type="number" name="registros_por_pagina" id="registros_por_pagina" 
                                   value="<?php echo $registros_por_pagina; ?>" 
                                   min="1" max="500" 
                                   style="width: 70px; padding: 4px 8px; border: 1px solid #ddd; border-radius: 4px; font-size: 12px; text-align: center;"
                                   placeholder="Cantidad">
                            <button type="submit" style="padding: 4px 8px; border: 1px solid #3498db; background: #3498db; color: white; border-radius: 4px; font-size: 12px; cursor: pointer;">
                                Aplicar
                            </button>
                            ...
                            <input type="hidden" name="pagina" value="1">
                            <?php if (isset($_GET['filtrado']) && $_GET['filtrado'] === 'true'): ?>
                                <input type="hidden" name="filtrado" value="true">
                            <?php endif; ?>
                            
                            <!-- Mantener filtros activos -->
                            <?php 
                            $filtros_url = ['paises', 'apellidos', 'asignados', 'gestiones', 'estados'];
                            foreach ($filtros_url as $filtro) {
            ...
            <div style="margin: 10px 20px; padding: 10px; background-color: #e8f4fd; border-radius: 4px; border-left: 4px solid #3498db;">
                ...
                <?php elseif ($search_field === 'gestion'): ?>
                    (solo en Gestión)
                <?php elseif ($search_field === 'auxiliar'): ?>
                    (solo en Auxiliar)
                <?php else: ?>
                    (en todos los campos)
                <?php endif; ?>
            ...
            <div class="table-container">
                <table class="leads-table">
                    ...
                    <tbody>
                        <?php
                        try {
                            // CONSULTA OPTIMIZADA con búsqueda
                            $query = "SELECT 
                                c.TP,
                                c.Nombre,
                                c.Apellido,
                            ...
                            // Guardar el orden actual de los TP para navegación
                            if (!isset($_SESSION['leads_navigation']) || isset($_GET['pagina']) || isset($_GET['search']) || isset($_GET['filtrado'])) {
                                $_SESSION['leads_navigation'] = [];
                            }
                            
                            if ($stmt->rowCount() > 0) {
                                while ($cliente = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                     // Guardar TP en sesión para navegación
                                    $_SESSION['leads_navigation'][] = $cliente['TP'];
                                    echo "<tr>";
                                    echo "<td><strong>" . htmlspecialchars($cliente['TP'] ?? '') . "</strong></td>";
                                    //echo "<td>" . htmlspecialchars($cliente['Nombre'] ?? '') . "</td>";
                                    echo '<td>
                                        <a href="detalles_lead.php?tp=' . urlencode($cliente['TP']) . '" class="link-lead">
                                            ' . htmlspecialchars($cliente['Nombre']) . '
                                        </a>
                                    </td>';
                                    echo "<td>" . htmlspecialchars($cliente['Apellido'] ?? '') . "</td>";
                                    echo "<td>" . htmlspecialchars($cliente['Numero'] ?? '') . "</td>";
                                    echo "<td title='" . htmlspecialchars($cliente['Correo'] ?? '') . "'>" . htmlspecialchars($cliente['Correo'] ?? '') . "</td>";
                                    ...
                                    echo "<td>" . htmlspecialchars($cliente['Asignado'] ?? '') . "</td>";
                                    ...
                                    echo "<td>
                                            <div class='action-buttons'>
                                                ...
                                                <button class='btn-action btn-edit' title='Editar' onclick='editClient(\"" . htmlspecialchars($cliente['TP'] ?? '') . "\")'>
                                ...
                                echo "<tr><td colspan='11' style='text-align: center; padding: 20px;'>";
                            ...
                            echo "<tr><td colspan='11' style='text-align: center; padding: 20px; color: #e74c3c;'>";
...
.leads-table th:nth-child(1), .leads-table td:nth-child(1) { width: 120px; } /* TP */
.leads-table th:nth-child(2), .leads-table td:nth-child(2) { width: 150px; } /* Nombre */
.leads-table th:nth-child(3), .leads-table td:nth-child(3) { width: 150px; } /* Apellido */
.leads-table th:nth-child(4), .leads-table td:nth-child(4) { width: 130px; } /* Teléfono */
.leads-table th:nth-child(5), .leads-table td:nth-child(5) { width: auto; min-width: 200px; } /* Email - Ancho automático */
.leads-table th:nth-child(6), .leads-table td:nth-child(6) { width: 100px; } /* Estado */
.leads-table th:nth-child(7), .leads-table td:nth-child(7) { width: 100px; } /* País */
.leads-table th:nth-child(8), .leads-table td:nth-child(8) { width: 150px; } /* Asignado a */
.leads-table th:nth-child(9), .leads-table td:nth-child(9) { width: 200px; } /* Última Gestión */
.leads-table th:nth-child(10), .leads-table td:nth-child(10) { width: 150px; } /* Fecha */
.leads-table th:nth-child(11), .leads-table td:nth-child(11) { width: 140px; } /* Acciones */

/* Para la columna de Email - permitir que se expanda */
...
/* Para la columna de Última Gestión */
...
/* Estilos para las notas con usuario */
.nota-item {
    background: #ffffff;
    border-left: 4px solid #3498db;
    padding: 15px;
...
/* Para notas sin descripción */
.nota-descripcion:empty::before {
    content: "Sin descripción detallada";
    color: #95a5a6;
    font-style: italic;
}
...
function viewClient(tp) {
    ...
    // URL de la petición
    const url = `ver_cliente.php?tp=${encodeURIComponent(tp)}`;
            ...
            modalContent.innerHTML = `
                <div style="text-align: center; padding: 40px; color: #e74c3c;">
                    ...
                    <div style="margin-top: 15px;">
                        <button onclick="window.open('ver_cliente.php?tp=${tp}', '_blank')" class="btn btn-secondary" style="margin: 5px;">
...
// Función para mostrar la información del cliente en el modal
function mostrarInformacionCliente(cliente, notas) {
        ...
        <div class="cliente-info">
            <div style="grid-column: 1 / -1;">
                ...
                <div class="info-group">
                    ...
                    <div class="info-value" style="font-size: 18px; font-weight: bold; color: #2c3e50;">${cliente.Nombre || 'N/A'}</div>
                </div>
            </div>
        </div>
    ...
    if (notas.length > 0) {
        notas.forEach(nota => {
            // Obtener información del usuario - ¡AHORA SÍ TIENES EL CAMPO "user"!
            const usuarioNombre = nota.user || 'Usuario no registrado';
            const fechaNota = nota.FechaUltimaGestion || 'N/A';
            const tipoGestion = nota.UltimaGestion || 'Nota';
                ...
                <div class="nota-item">
                    <div class="nota-header">
                        <div class="nota-fecha">
                            <i class="far fa-calendar"></i> ${formatearFecha(fechaNota)}
                        </div>
...
function formatearFecha(fecha) {
    ...
    try {
        ...
        if (isNaN(fechaObj.getTime())) return fecha; // Si no es una fecha válida
...
function makeCall(numero, extension) {
    ...
    const url = `llamada.php?numero=${encodeURIComponent(numero)}&extension=${encodeURIComponent(extension)}`;
...
function cargarEstados() {
    ...
    return fetch('obtener_estados.php')
        .then(response => {
...
function guardarEdicionCliente() {
    ...
    fetch('guardar_edicion_cliente.php', {
        method: 'POST',
        body: formData
    })
    .then(response => response.json())
...
function cargarOpcionesFiltro() {
    fetch('obtener_opciones_filtro.php')
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
// Función para el selector de registros por página
function inicializarSelectorRegistros() {
    const inputRegistros = document.getElementById('registros_por_pagina');
    const formRegistros = document.getElementById('formRegistrosPorPagina');
    
    if (inputRegistros && formRegistros) {
        // Validar entrada al escribir
        inputRegistros.addEventListener('input', function() {
        ...
        inputRegistros.addEventListener('keypress', function(e) {
            if (e.key === 'Enter') {
...
// Inicializar el selector de registros cuando cargue la página
document.addEventListener('DOMContentLoaded', function() {
