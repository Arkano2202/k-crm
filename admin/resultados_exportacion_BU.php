...
function construirConsulta($filtros, $contar = false) {
    ...
    if ($contar) {
        ...
        $query = "SELECT * FROM clientes $where_clause ORDER BY Numero DESC";
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['exportar'])) {
    ...
    if (empty($leads_seleccionados)) {
        ...
        $leads_a_exportar = array_filter($leads, function($lead) use ($leads_seleccionados) {
            return in_array($lead['TP'], $leads_seleccionados);
        });
    }
