...
echo "console.log('Grupo:', '" . ($usuario_actual['id'] ?? 'NO DEFINIDO') . "');";
echo "console.log('Es TL:', " . (isTL() ? 'true' : 'false') . ");";
echo "</script>";

// Obtener citas para el día específico CON FILTRO POR GRUPO
try {
    ...
    $query = "SELECT c.id, c.titulo, c.descripcion, c.fecha, c.hora, u.nombre as usuario_nombre, u.usuario as usuario_login
              ...
              WHERE c.fecha = :fecha_dia";
    
    // MODIFICACIÓN: Filtrar por grupo del usuario de sesión para TODOS los usuarios (incluyendo TL)
    $query .= " AND c.grupo = :usuario_grupo";
    ...
    // MODIFICACIÓN: Pasar el grupo del usuario como parámetro para TODOS
    $grupo_usuario = $usuario_actual['id'];
    $stmt->bindParam(':usuario_grupo', $grupo_usuario);
    
    // DEBUG
    echo "<script>";
    echo "console.log('=== DEBUG FILTRO ===');";
    echo "console.log('Usuario es TL pero aplicando filtro por grupo:', '" . ($grupo_usuario ?? 'NO DEFINIDO') . "');";
...
<div class="main-content">
    ...
    <div class="content-area">
        ...
        <!-- Leyenda de colores para agentes -->
        <?php if (isTL() && !empty($agentes_unicos)): ?>
            ...
            <?php else: ?>
                <div class="daily-appointments">
                    <?php foreach ($citas as $cita): ?>
                        ...
                        $color_agente = isTL() && $cita['usuario_nombre'] ? 
                        ...
                        <div class="daily-appointment" onclick="filtrarTP('<?php echo $tp_completo; ?>')" 
                            ...
                            <div class="appointment-content">
                                ...
                                <div class="appointment-meta">
                                    ...
                                    <?php endif; ?>
                                    <?php if (isTL() && $cita['usuario_nombre']): ?>
                                        <span class="appointment-user">Agente: <?php echo htmlspecialchars($cita['usuario_nombre']); ?></span>
                                    <?php endif; ?>
                                </div>
                            </div>
