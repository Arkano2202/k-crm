...
.lead-card .label { font-size:11px;color:#95a5a6;text-transform:uppercase;font-weight:600; }
.lead-card .value {
    font-size:16px;
    font-weight:600;
    margin-top:6px;
...
#formNota .nota-field { display:flex; flex-direction:column; }
#formNota .nota-field-full { grid-column:1/-1; }
...
.then(data=>{
    ...
    numeroLead = c.Numero; // SIEMPRE guardar el número real
...
function hacerLlamada(){
    ...
    fetch(`../admin/llamada.php?numero=${encodeURIComponent(numeroLead)}&extension=${encodeURIComponent(extensionUsuario)}`);
}
function mostrarFormularioNota(){ document.getElementById('formNota').style.display='block'; cargarEstados(); }
function cancelarNota(){ document.getElementById('formNota').style.display='none'; }
function cargarEstados(){
    fetch('../admin/obtener_estados.php').then(r=>r.json()).then(d=>{
        const s=document.getElementById('gestionSelect');
        s.innerHTML='<option value="">Seleccionar gestión</option>';
        d.estados.forEach(e=>{
            const o=document.createElement('option');
            o.value=e.Estado; o.textContent=e.Estado;
            s.appendChild(o);
        });
    });
}
function guardarNota(){
    ...
    fd.append('tp',tp); fd.append('gestion',g); fd.append('descripcion',d);
    fetch('../admin/guardar_nota.php',{method:'POST',body:fd})
    .then(r=>r.json()).then(x=>{ if(x.success)location.reload(); else alert(x.error); });
}
</script>
