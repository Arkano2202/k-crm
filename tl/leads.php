...
// FILTRO POR GRUPO DEL USUARIO - MODIFICADO
try {
    $user_id = getCurrentUserId();
    
    // Obtener clientes asignados a usuarios del mismo grupo
    $where_conditions[] = "c.Asignado IN (SELECT nombre FROM users WHERE grupo = :usuario_id)";
    $params[':usuario_id'] = $user_id;
    
    // Debug opcional
    echo "<!-- Debug: User ID: $user_id, Filtro por grupo aplicado -->";
    
} catch (Exception $e) {
    // Fallback: mostrar solo los asignados al usuario actual
    $where_conditions[] = "c.Asignado = :usuario_session";
    $params[':usuario_session'] = $usuario_actual['nombre'];
    echo "<!-- Error en filtro por grupo, usando fallback: " . $e->getMessage() . " -->";
}
                        ...
                        try {
                            ...
                            if ($stmt->rowCount() > 0) {
                                while ($cliente = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                    ...
                                    echo "<td>
                                        <a href='detalles_lead.php?tp=" . urlencode($cliente['TP']) . "' 
                                    ...
                                    // Teléfono oculto
                                    echo "<td title='Información confidencial'>";
                                    echo "<span style='color: #bdc3c7; letter-spacing: 2px;'>••••••••••</span>";
                                    echo "</td>";
...
/* Mejoras para las notas con nombre de usuario */
.nota-fecha {
    font-size: 12px;
    color: #7f8c8d;
    margin-bottom: 5px;
            ...
            html += `
                <div class="nota-item">
                    <div class="nota-fecha">
                        <strong>Fecha:</strong> ${nota.FechaUltimaGestion || nota.fecha_creacion || 'N/A'}
                        ${nota.usuario_nombre ? `<span style="margin-left: 10px; color: #7f8c8d;">| <strong>Por:</strong> ${nota.usuario_nombre || nota.usuario_login || 'Sistema'}</span>` : ''}
                    </div>
                    <div class="nota-tipo">
                        ${nota.UltimaGestion || nota.tipo_gestion || 'Nota'}
                    </div>
                    <div class="nota-descripcion">
                        ${nota.Descripcion || nota.descripcion || 'Sin descripción'}
                    </div>
                    <div class="usuario-nota">
                        ${nota.user || nota.user || 'Sistema'}
                    </div>
                </div>
            `;
        });
